...
Rysunek 7.4 z [1], zamieszczony poniżej, pokazuje wartości wariancji tego estymatora w funkcji parametru $\epsilon$, przy $S(0)=K=100$, $\sigma=0.3$, $r=0.05$, $T=0.25$. Przypadek $\epsilon=0$ odpowiada użyciu wyłącznie metody ilorazu wiarygodności. Wartości $\epsilon$ bliskie zera, dają w wyniku dużą wariancję, z winy $\epsilon$ w mianowniku estymatora, ale przy dużych $\epsilon$ wariancja jest mała. Minimum występuje dla zaskakująco dużej wartości $\epsilon\approx35$.
