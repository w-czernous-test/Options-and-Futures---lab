...
```{r}
# Załóżmy, że mamy już wyznaczone wszystkie węzły siatki X[i,j] 
# oraz dane funkcje:
# -wypłaty (niezależną od i), tilde_h(x);
# -wagową (patrz rozdział "Wagi estymatora dolnego"), W(i,k,x);
# a także stałą dyskonta ($D_{i,i+1}(x) = D$).
# Załóżmy też, że dopuszczamy możliwość realizacji opcji również w chwili $t_0$.
find_hatV_mj = function(j) {
  hatV[m,j] = tilde_h(X[m,j])
}
find_hatV_ij = function(i,j) {
  C = c()
  X_ij = X[i,j]
  for (k in 1:b) {
    Wi_jk = W(i,k,X_ij)
    C[k] = Wi_jk * D * hatV[i+1,k]
  }
  hatV[i,j] = max(tilde_h(X_ij), mean(C))
}
