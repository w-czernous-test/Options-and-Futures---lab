##### TODO na ocenę 4: dodać "control variate" w estymatorze dolnym (patrz [5], str. 15-16, "Path estimator enhancements") ###############

##### TODO na ocenę 5: dodać zewnętrzny "control variate" dla obu estymatorów (patrz [5], "outer controls", str. 15) ###############
...
d = 1   # Wymiar. Stan rynku jest wektorem cen: $X = S = (S_1,...,S_d)$.
...
h_i = function(i,x) { return ( exp(-r*i*dt) * max(0, x - K) ) } # Opcja typu "call" dla jednego aktywa.
...
european_option_price_dt = function(S_0) {
  d = (log(S_0/K) + r*dt) / sigsqrt
  return ( pnorm(d + sigsqrt/2) * S_0 - pnorm(d - sigsqrt/2) * K * exp(-r*dt) )
}
inner_control_variate_1_value = function(x) {return (h_i(1,x))}
inner_control_variate_1 = function(x) {return (european_option_price_dt(x))} # E( h_1( S_{t+1} ) | S_t = x )
